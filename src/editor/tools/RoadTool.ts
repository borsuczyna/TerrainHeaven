...
export default class RoadTool implements Tool {
    ...
    private cancelPreview(): void {
        if (this.previewRoad) {
            this.scene.instance.remove(this.previewRoad.mesh);
            this.previewRoad = null;
        }
        this.startNode = null;
        this.dragging = false;
    ...
    private findHitNode(): WorldNode | null {
        ...
        for (const child of this.scene.instance.children) {
            if (this.previewRoad && child === this.previewRoad.mesh) continue;
    ...
    private onMouseMove = (e: MouseEvent): void => {
        if (!this.dragging || !this.previewRoad) return;
        const endPoint = this.getEndPoint(e);
        if (endPoint) {
            this.previewRoad.nodeB.update(endPoint);
        }
    };
    ...
    onMouseDown(e: MouseEvent): boolean {
        ...
        // Create preview road at start position (both nodes at same spot)
    ...
    private handleMouseUp = (e: MouseEvent): void => {
        ...
        if (this.startNode) {
            ...
            startPos = this.previewRoad.nodeA.mesh.position.clone();
        }
