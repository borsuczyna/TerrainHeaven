...
@singleton()
export default class TerrainCutSplineTool implements Tool {
    ...
    private previewSpline: TerrainCutSpline | null = null;
    ...
    private cancelPreview(): void {
        if (this.previewSpline) {
            this.scene.remove(this.previewSpline);
            this.previewSpline = null;
        }
        this.startNode = null;
        this.dragging = false;
    ...
    private findHitNode(): WorldNode | null {
        ...
        for (const child of this.scene.instance.children) {
            if (this.previewSpline && child === this.previewSpline.mesh) continue;
    ...
    private onMouseMove = (e: MouseEvent): void => {
        if (!this.dragging || !this.previewSpline) return;
        const endPoint = this.getEndPoint(e);
        if (endPoint) {
            this.previewSpline.nodeB.update(endPoint);
        }
    };
    ...
    onMouseDown(e: MouseEvent): boolean {
        ...
        this.previewSpline = new TerrainCutSpline(startPos.clone(), startPos.clone());
        this.scene.add(this.previewSpline);
        ...
        if (this.startNode?.parent) {
            ...
            if (nodeIndex >= 0 && !parentElement.isConnected(nodeIndex)) {
                this.previewSpline.connect(0, parentElement, nodeIndex);
            }
        }
    ...
    private handleMouseUp = (e: MouseEvent): void => {
        if (e.button !== 0 || !this.dragging || !this.previewSpline) return;
        ...
        if (this.startNode) {
            ...
            startPos = this.previewSpline.nodeA.mesh.position.clone();
        }
        ...
        this.previewSpline.nodeB.update(endPos);

        if (hitNode?.parent) {
            const parentElement = hitNode.parent;
            const nodeIndex = parentElement.getNodeIndex(hitNode);
            if (nodeIndex >= 0 && !parentElement.isConnected(nodeIndex)) {
                this.previewSpline.connect(1, parentElement, nodeIndex);
            }
        }

        this.history.record('Add Terrain Cut Spline');
        this.reset();
    };
}
